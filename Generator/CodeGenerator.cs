...
public class CodeGenerator(ChannelReader<ClassDefinition> channel)
{
    ...
    public async Task GenerateCode()
    {
        ...
        await foreach (var typeDefinition in channel.ReadAllAsync())
        {
            ...
            try
            {
                ...
                GenerateClassCode(writer, namespaceName, className, superClass, properties, methods, inheritedPropertes, inheritedMethods);
            }
            catch (Exception ex)
            {
    ...
    private void GenerateClassCode(TextWriter writer, string namespaceName, string className, string superClass, List<PropertyDefinition> properties, List<MethodDefinition> methods, List<PropertyDefinition> inheritedPropertes, List<MethodDefinition> inheritedMethods)
    {
        writer.WriteLine("""
            /**************************************************************
             *                                                            *
            ...
            #nullable enable
            """);
        writer.WriteLine($"namespace System.Management.Types.{namespaceName};");
        writer.WriteLine();
        writer.WriteLine($"public partial record class {className}(ManagementObject ManagementObject) : {superClass}(ManagementObject)");
        writer.WriteLine("{");

        properties.ForEach(p => GenerateProperty(writer, p, inheritedPropertes.Any(ip => ip.Name.Equals(p.Name)), namespaceName));

        methods.ForEach(m => GenerateMethod(writer, m, inheritedMethods.Any(im => Match(m, im)), namespaceName));
    ...
    private void GenerateProperty(TextWriter writer, PropertyDefinition property, bool hidesInherited, string namespaceName)
    {
        ...
        writer.Write(GetPrintableType(property, namespaceName));
        if (property.IsArray)
        {
            writer.Write("[]");
        }
        ...
        if (property.Type == CimType.DateTime)
        {
            if (property.IsArray)
            {
                writer.Write("ManagementObject.GetDateTimePropertyValues(nameof(");
            }
            else
            {
                writer.Write("ManagementObject.GetDateTimePropertyValue(nameof(");
            }
            writer.Write(property.Name);
            writer.Write("))");
        }
        else
        {
            ...
            writer.Write(")ManagementObject[nameof(");
            writer.Write(property.Name);
            writer.Write(")]");
        }
        writer.WriteLine(";");
    ...
    private void GenerateMethod(TextWriter writer, MethodDefinition method, bool hidesInherited, string namespaceName)
    {
        writer.Write("    public ");
        if (hidesInherited)
        {
        ...
        writer.Write("Task<");
        writer.Write(GetPrintableType(method.ReturnType));
        writer.Write("> ");
        ...
        foreach (var parameter in method.Parameters)
        {
            ...
            writer.Write(GetPrintableType(parameter, namespaceName));
        ...
        writer.Write("    => ManagementObject.InvokeMethod(nameof(");
        writer.Write(method.Name);
        writer.WriteLine("),");
